...
var manifest = require('asset-builder')('./ui/assets/manifest.json');
...
gulp.task('static', function() {
    ...
    browserSync.init({
        ...
        files: ['ui/*.html', 'ui/**/*.css', '*ui/**/*.js'],
        notify: false
    });
    gulp.watch([path.source + 'styles/**/*'], ['styles']);
    gulp.watch([path.source + 'scripts/**/*'], ['jshint', 'scripts']);
    gulp.watch([path.source + 'fonts/**/*'], ['fonts', 'styles']);
    gulp.watch([path.source + 'images/**/*'], ['images']);
    gulp.watch([path.source + 'styles/app/icons/svgs-for-iconfont/**/*'], ['icons']);
    gulp.watch([path.source + 'styles/app/icons/svgs-for-sprite/**/*'], ['svgSprite']);
    gulp.watch(['bower.json', 'ui/assets/manifest.json'], ['build']);
});
...
// ### SVGSprite
// `gulp svgSprite` - Make svg sprite from svg icons
var svgSprite = require('gulp-svg-sprite');
var svg2png   = require('gulp-svg2png');
var svgo      = require('gulp-svgo');
// converting svgs to one sprite
gulp.task('svgSprite', function(tmp) {
    gulp.src(['ui/assets/styles/app/icons/svgs-for-sprite/*.svg', 'ui/assets/styles/app/icons/svgs-for-sprite/*/*.svg'])
        // optimize svgs
        .pipe(svgo())
        // make sprite
        .pipe(svgSprite({
            "mode": {
                "css": {
                    ...
                    "render": {
                        "scss": {
                            "dest": "assets/styles/app/autogenerated/_sprite.scss",
                            "template": "ui/assets/styles/lib/sprite/sprite-template.scss"
                        }
                    }
                }
            }
...
gulp.task('pngSprite', function() {
    gulp.src('ui/assets/images/sprite.svg')
        .pipe(svg2png())
        .pipe(gulp.dest('ui/assets/images/'));
});
...
// ui\assets\images\svgs-for-iconfont
var paths = {
    templates: 'ui/assets/styles/lib/iconfont/',
    icons: 'ui/assets/styles/app/icons/svgs-for-iconfont/',
    fonts: 'ui/assets/fonts/',
    scss: 'ui/assets/styles/app/autogenerated/',
    scssIconfont: 'ui/assets/styles/app/autogenerated/'
};
    ...
    gulp.src([paths.icons + '*.svg'])
        ...
        .on('glyphs', function(glyphs) {
            gulp.src(paths.templates + '_iconfont.scss')
                ...
                .pipe(gulp.dest(paths.scssIconfont));
        })
        .pipe(svg2ttf())
